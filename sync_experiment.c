...
int calculate_sync_drift(void *data)
{
	...
	while (!kthread_should_stop())
		...
		/*if(atomic_read(&wake_up_signal_sync_drift[cpuID]) == 1){
			//atomic_set(&wake_up_signal_sync_drift[cpuID],0);
			curr_sync_task_finished_flag[cpuID] = 0;
		}
		else{
			//set_current_state(TASK_INTERRUPTIBLE);
			//run_cpu = get_cpu();
			//printk(KERN_INFO "TimeKeeper: #################Sync Drift Thread waiting for  %d on run_cpu %d\n", cpuID,run_cpu);
			schedule();
			
			//wait_event(per_cpu_sync_task_queue[cpuID],curr_sync_task_finished_flag[cpuID] == 1);
			//atomic_set(&wake_up_signal_sync_drift[cpuID],0);
			//curr_sync_task_finished_flag[cpuID] = 0;
	...
	while (!kthread_should_stop())
		...
		if (number_of_heads > 0) {
			...
			for (i=0; i<number_of_heads; i++) {
				...
				//wake_up(&per_cpu_sync_task_queue[i]);
		...
		if (proc_num == 0 && experiment_stopped == RUNNING) {
			...
			continue;
                ...
                set_current_state(TASK_INTERRUPTIBLE);	
		/*if(atomic_read(&woke_up_catchup_Task) == 1){
			atomic_set(&woke_up_catchup_Task,0);
		}
		else{
			atomic_set(&catchup_Task_finished,1); // *** Added new
        	        schedule();
			atomic_set(&catchup_Task_finished,0); // *** Added new
			atomic_set(&woke_up_catchup_Task,0);
		}*/
	...
	do {
		if (t->freeze_time == 0 && t->wakeup_time ==  0)
			...
			if(last_ppp != -1){
				...
				//if(rem != 0)
				//	t->past_virtual_time = t->past_virtual_time + rem;
			}
			...
			if(t->freeze_time == -1 && t->wakeup_time == 0){
				...
				if(t->wakeup_time == 0){
					...
					//if(rem != 0)
					//	t->past_virtual_time = t->past_virtual_time + rem;
					//t->freeze_time = freeze_time; // reset it anyway.
				}
			}
		}		
...
Freezes all children associated with a container
***/
int stop_all(struct task_struct *aTask, int CPUID) {
        struct list_head *list;
        struct task_struct *taskRecurse;
        struct dilation_task_struct *dilTask;
	struct task_struct *me;
	...
	if(CPUID ==1 && aTask->state != TASK_STOPPED){
		printk(KERN_INFO "TimeKeeper : ERROR : PID : %d Not dequeued yet\n", aTask->pid);
		...
		kill(taskRecurse, SIGSTOP, NULL);		// *** Added		
                stop_all(taskRecurse, CPUID);
        }
	return 0;
}
...
int unfreeze_proc_exp_recurse(struct dilation_task_struct *aTask, s64 expected_time) {
	...
	if(schedule_list_size(aTask) == 1){
		...
		//set_all_freeze_times_recurse(aTask->linux_task, now_ns,now_ns - start_ns);
